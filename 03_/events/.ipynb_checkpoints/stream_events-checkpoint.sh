#!/bin/sh
"""
This is a bash shell script counter.  It executes the commands to generate events using Apache Bench.  The counter value is incorporated into the host ID to create unique users.  The sleep command halts the while loop for five seconds.  It takes about five minutes to generate two hundred events.
"""
count=1
while [ $count -lt 51 ]
do
